...
typedef struct _router {
    int id;
    volatile long controlword; // control word used to select routing choice,
    ...
    int function[rtMAXPORTS][rtMAXCALLS]
                [rtNVAR]; // identifier for function to be called
    int callid[rtMAXPORTS][rtMAXCALLS][rtNVAR]; // 'id' to be used in the call
    int nstreams[rtMAXPORTS]; // number of data streams interleaved for each
                              // port
...
PORT void xrouter(void* ptr, int id, int port, int nsamples, SAMPLETYPE* data);
