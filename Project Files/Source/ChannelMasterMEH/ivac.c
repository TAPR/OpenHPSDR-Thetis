...
PORT void create_ivac(int id, int run,
    ...
    create_resamps(a);
    {
        ...
        a->ndoubles = 192000;
        a->pdoubles = malloc(192000 * sizeof(double));
    }
    pvac[id] = a;
}
...
static inline void* doubles_to_floats(
    const void* input, unsigned int frameCount, IVAC pvac, int id) {

    float* doubles = (float*)input;
    float* my_floats = (float*)input;
    float* floats = my_floats;
    double* my_input = (double*)input;

    for (unsigned int i = 0; i < frameCount * pvac->num_channels; ++i) {
        *my_floats++ = (float)*my_input++;
    }

    if (id == 0) {
        //dump_to_file(doubles, frameCount * pvac->num_channels * sizeof(double));
    }
    return floats;
}

static inline void* floats_to_doubles(
    const void* input, unsigned int frameCount, IVAC pvac, int id) {

    float* floats = (float*)input;
    float* my_floats = (float*)input;
    float* my_input = (float*)input;
    double* doubles = pvac->pdoubles;
    double* my_doubles = pvac->pdoubles;

    if (id == 0) {
        //dump_to_file2(input, frameCount * pvac->num_channels * sizeof(double));
    }

    assert(frameCount * pvac->num_channels <= (unsigned int)pvac->ndoubles);

    for (unsigned int i = 0; i < frameCount * pvac->num_channels; ++i) {
        *my_doubles++ = (double)*my_input++;
    }
...
#pragma warning(default : 4311)
    ...
    if (!a->run) return 0;
    //double* my_doubles = floats_to_doubles(in_ptr, frameCount, a, id);
    xrmatchIN(a->rmatchIN, in_ptr); // MIC data from VAC
    xrmatchOUT(a->rmatchOUT, out_ptr); // audio or I-Q data to VAC

    //out_ptr = doubles_to_floats(out_ptr, frameCount, a, id);
...
PORT void SetIVACaudioRate(int id, int rate) {
    ...
    if (rate != a->audio_rate) {
        ...
        destroy_aamix(a->mixer, 0);
        {
            ...
            // I know, I know, can leak, whatever... for now
            a->ndoubles = 192000;
            a->pdoubles = malloc(192000 * sizeof(double));
        }
        destroy_resamps(a);
