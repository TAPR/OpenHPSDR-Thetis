                ...
                switch (rc) {
                    ...
                    case 1026: // 1440 bytes 16-bit mic samples
                        for (i = 0, k = 0; i < prn->mic.spp; i++, k += 2) {
                            ...
                            prn->TxReadBufp[2 * i]
                                = (float)const_1_div_2147483648_
                                * (SAMPLETYPE)(prn->ReadBufp[k + 0] << 24
                                    | prn->ReadBufp[k + 1] << 16);
                    ...
                    case 1041:
                        for (i = 0, k = 0; i < prn->rx[0].spp; i++, k += 6) {
                            prn->RxReadBufp[2 * i + 0]
                                = (float)const_1_div_2147483648_
                                * (SAMPLETYPE)(prn->ReadBufp[k + 0] << 24
                                    | prn->ReadBufp[k + 1] << 16
                                    | prn->ReadBufp[k + 2] << 8);

                            prn->RxReadBufp[2 * i + 1]
                                = (float)const_1_div_2147483648_
                                * (SAMPLETYPE)(prn->ReadBufp[k + 3] << 24
                                    | prn->ReadBufp[k + 4] << 16
                                    | prn->ReadBufp[k + 5] << 8);
                        }
