...
namespace Thetis
{
    ...
    internal static class MeterManager
    {
        ...
        static MeterManager()
        {
            ...
            // test
            clsMeter cm = new clsMeter(1, "test1meter", (int)MeterType.RX, 1f, 1f);
            _meters.Add(cm.Name, cm);

            //cm = new clsMeter(2, "test2meter", (int)MeterType.RX, 1f, 1f);
            //_meters.Add(cm.Name, cm);

            cm = new clsMeter(1, "test3meter", (int)MeterType.MOX | (int)MeterType.TUNE | (int)MeterType.TWOTONE, 1f, 1f);
            _meters.Add(cm.Name, cm);
            //
        ...
        public static int QuickestUpdateInterval(int rx, bool mox)
        {
            ...
            if (!mox)
            {
                foreach (KeyValuePair<string, clsMeter> kvp in _meters.Where(kvp => kvp.Value.RX == rx && (kvp.Value.TypeBitield & (int)MeterType.RX) == (int)MeterType.RX))
                {
        ...
        private class clsMeterItem
        {
            ...
            public bool RequiresUpdate
            {
                get
                {
                    ...
                    bool bNeedsUpdate = _stopwatch.ElapsedMilliseconds >= _msUpdateInterval;
                    if(bNeedsUpdate) _stopwatch.Stop();
        ...
        private class clsMeter
        {
            ...
            public int TypeBitield { get { return _TypeBitfield; } }
            public string ID { get { return _sGuid; } }
        ...
        private class DXRenderer
        {
            ...
            {
                if(_displayTarget != null) _displayTarget.Resize -= target_Resize;
            ...
            private void drawMeters()
            {
                foreach (KeyValuePair<string, clsMeter> mkvp in _meters)
                {
                    ...
                    if (m.RX == _rx)
                    {
                        // this rx
                        if (
                            ((m.TypeBitield & (int)MeterType.RX) == (int)MeterType.RX && !_mox) ||
                            ((m.TypeBitield & (int)MeterType.MOX) == (int)MeterType.MOX && _mox)
                            )
                        {
            ...
            private void renderNeedle(SharpDX.RectangleF rect, clsMeterItem mi)
            {
                ...
                if (ni.ShowHistory || ni.Setup)
                {
                    ...
                    float endMinX = startX + (float)(Math.Cos(angMin + degToRad(rotation)) * radius);
                    float endMinY = startY + (float)(Math.Sin(angMin + degToRad(rotation)) * radius);
                    float endMaxX = startX + (float)(Math.Cos(angMax + degToRad(rotation)) * radius);
                    float endMaxY = startY + (float)(Math.Sin(angMax + degToRad(rotation)) * radius);
                ...
                float endX = startX + (float)(Math.Cos(ang + degToRad(rotation)) * radius);
                float endY = startY + (float)(Math.Sin(ang + degToRad(rotation)) * radius);
